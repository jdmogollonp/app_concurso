...
const contestsApi = (app) => {
  ...
  router.post('/administrartor/new', verifyToken, async (req, res) => {
    try {
      ...
      const { url, start_date, end_date, description } = JSON.parse(req.body.data);
      if (!url || !start_date || !end_date || !description) {
        res.status(400).json({
          errors: [`Todos los datos son requeridos`]
        });
      }
      const imageFileName = `${bannerContests}/${url}_${new Date().getTime()}${path.extname(imageFile.name)}`
      console.log('Segumos entrando');
      // imageFile.mv(`${__dirname}/../../${videoFileName}`).then(async () => {
      //
